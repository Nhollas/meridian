...
describe("createDockerRuntime", () => {
	beforeEach(() => {
		...
		delete process.env["MERIDIAN_AUTH_ISSUER"];
	...
	it("passes configured auth environment into new containers", async () => {
		...
		process.env["MERIDIAN_AUTH_ISSUER"] = "https://issuer.example";
		const runtime = createDockerRuntime();
		mockExecFileSequence([
			{ exitCode: 1, stderr: "No such container" },
		...
		expect(childProcessMocks.execFile).toHaveBeenNthCalledWith(
			...
			expect.arrayContaining([
				...
				"-e",
				"MERIDIAN_AUTH_ISSUER=https://issuer.example",
			]),
			{ timeout: 30000 },
			expect.any(Function),
		);
	...
	it("uses default auth environment when the shell does not provide it", async () => {
		...
		expect(childProcessMocks.execFile).toHaveBeenNthCalledWith(
			...
			expect.arrayContaining([
				...
				"MERIDIAN_AUTH_CLIENT_ID=meridian-cli",
				"-e",
				"MERIDIAN_AUTH_ISSUER=https://cicd-meridian-oauth-server-shadow.vassily.io/realms/meridian",
			]),
			{ timeout: 30000 },
			expect.any(Function),
		);
