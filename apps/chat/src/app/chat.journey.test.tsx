...
describe("Chat UI - broadband comparison journey", () => {
	test("renders a multi-turn broadband comparison with mixed tool types", async ({
		...
		const turn1Factory = createChatEventFactory({ turnId: "turn-1" });
		const turn2Factory = createChatEventFactory({ turnId: "turn-2" });
		...
		browserWorker.use(
			http.post(
				...
				withJsonBody({ message: "Help me compare broadband" }, () => {
					queueMicrotask(() => {
						sseStream.emit(
							turn1Factory.create("tool.completed", {
								toolCall: {
									id: "tc-1",
									input: "{}",
						...
						sseStream.emit(
							turn1Factory.create("turn.completed", {
								content:
					...
					return createChatAcceptedResponse("turn-1");
				}),
			),
			http.post(
				"http://localhost:3201/api/chat",
				withJsonBody({ message: "test@test.com, PE9 UUU" }, () => {
					queueMicrotask(() => {
						sseStream.emit(
							turn2Factory.create("tool.completed", {
								toolCall: {
									id: "tc-6",
									input:
										'{"command":["meridian","auth","status","--json"],"timeoutMs":120000}',
						...
						sseStream.emit(
							turn2Factory.create("turn.completed", {
								content:
					...
					return createChatAcceptedResponse("turn-2");
				}),
			),
		);
